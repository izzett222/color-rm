...
import { roadmapProjects } from "~/data/data";
...
const RoadmapSearch: React.FC = () => {
  ...
  const categories = useMemo(() => {
    const cats = Array.from(
      new Set(roadmapProjects.map((item) => item.category))
    );
    return ["all", ...cats];
  }, []);
  ...
  const suggestedTags = useMemo(() => {
    ...
    const relevantItems = roadmapProjects.filter(
      (item) =>
  ...
  return (
    <div
      className={`max-w-6xl mx-auto p-6 space-y-6 ${!searchQuery ? "flex flex-col items-center justify-center min-h-[80vh]" : ""}`}
    >
      {/* Header */}
      <div className={`text-center space-y-2 ${!searchQuery ? "" : ""}`}>
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          ...
          vote on priorities, and track progress, all designed to boost
          productivity and foster innovation within Rwanda's growing talent
          ecosystem.
        </p>
      </div>
        ...
        <>
          ...
          <div className="flex items-center justify-between">
            <p className="text-gray-600">
              {filteredRoadmaps.length} result
